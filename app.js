...
// World News page
app.get('/world', catchAsync( async function(req, res, next){
...
app.get('/technology', catchAsync( async function(req, res, next){
...
// Science News page
app.get('/science', catchAsync( async function(req, res, next){
...
app.get('/environment', catchAsync( async function(req, res, next){
...
// Searched News page
app.get('/search', catchAsync( async function(req, res, next) {
    const {q} = req.query;
    console.log(q);
...
app.get('/loadmore', catchAsync( async function(req, res, next){
...
async function getRelated(item, id) {
    ...
    while(related.length < 4 && ind <= 9) {
        ...
        if(relates) {
            ...
            related = Array.from(new Set(related.map(a => a.ObjectId))).map(id => { return related.find(a => a.ObjectId === id)}); // remove duplicates from the array
        }
        ind++;
    }
    return related;
...
app.get('/world/:id', catchAsync( async function(req, res, next){
...
app.get('/technology/:id', catchAsync( async function(req, res, next){
    const result = await axios.get(`http://${paperboy_back}:${paperboy_back_port}/api/summary?id=${req.params.id}`);
    const item = result.data;
    let related = await getRelated(item, req.params.id);
...
// Technology news show page
app.get('/science/:id', catchAsync( async function(req, res, next){
    const result = await axios.get(`http://${paperboy_back}:${paperboy_back_port}/api/summary?id=${req.params.id}`);
    const item = result.data;
    let related = await getRelated(item, req.params.id);
...
app.get('/environment/:id', catchAsync( async function(req, res, next){
    const result = await axios.get(`http://${paperboy_back}:${paperboy_back_port}/api/summary?id=${req.params.id}`);
    const item = result.data;
    let related = await getRelated(item, req.params.id);
...
app.get('/search/:id', catchAsync( async function(req, res, next){
    const {q} = req.query;
    const result = await axios.get(`http://${paperboy_back}:${paperboy_back_port}/api/summary?id=${req.params.id}`);
    const item = result.data;
    let related = await getRelated(item, req.params.id);
...
app.use((err, req, res, next) => {
    ...
    else if (err.message == 'Request failed with status code 404') err.message = 'Something went wrong with your request. Please check your url and try again.';
